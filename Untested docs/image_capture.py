...
if __name__ == "__main__":
    ...
    # ###################################################    # rs_color_frames = []
    # rs_depth_frames = []
    # timestamps = []    # for i in range(1000):
    #     print(“i:“,i)    #     color_frame, depth_frame = realsense.get_single_rgbd_frame()
    #     rs_color_frames.append(color_frame)
    #     rs_depth_frames.append(depth_frame)
    #     timestamps.append(time.time())    # path=‘./rs_data_test’
    ...
    # os.makedirs(depth_output_directory, exist_ok=True)    # for i, (color_frame, depth_frame, timestamp) in enumerate(zip(rs_color_frames, rs_depth_frames, timestamps)):
    #     color_image = np.asanyarray(color_frame.get_data())
    #     depth_image = np.asanyarray(depth_frame.get_data())    #     color_filename = os.path.join(color_output_directory, f"color_frame_{i}_{timestamp}.png")
    #     depth_filename = os.path.join(depth_output_directory, f"depth_frame_{i}_{timestamp}.png")    #     cv2.imwrite(color_filename, color_image)
    #     cv2.imwrite(depth_filename, depth_image)    #     print(f"(Writing) Realsense frame {i}“)    # print(“Realsense image writing done.“)###################################################    # start_time = time.time()
    # total_frame=900    # for i in range(total_frame):
    #     print(“frane_id:“,i)
    #     realsense.write_single_frame(frame_id=i)
    #     # time.sleep(1)    # frame_rate = (total_frame-1)/(time.time() - start_time)
    # print(“relsense framerate:“, frame_rate)